...
<template>
  <App>
    <ul class="flex space-x-2 rtl:space-x-reverse">
      <li>
        <Link :href="'/roles/create'" class="btn btn-primary btn-sm float-end">
    ...
    <div class="p-4 sm:p-8 bg-white shadow sm:rounded-lg">
      ...
      <div class="table-responsive">
        <table>
          ...
          <tbody v-for="(role, index) in roles.data">
            <tr>
              <td>{{ index+1 }}</td>
              <td align="right">{{ role.name }}</td>
              <td align="right">{{ role.guard_name }}</td>
              <td style="float: right">
                <div class="flex gap-4">
                  <Link :href="'/roles/' + role.id + '/edit'" method="get" v-tippy:edit>
                  ...
                  <Link :href="'/roles/' + role.id" method="delete"  as="button" v-tippy:delete>
                  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"
                    class="w-5 h-5">
